...
const EditPostPage = () => {
        ...
        const fetchPost = async () => {
            try {
                ...
                reset({
                    ...
                    priority: post.priority as any || 'low_priority',
    ...
    const onSubmit = async (data: PostFormData) => {
        ...
        try {
            // Partial payload pattern: Only send what changed for performance
            const dirtyKeys = Object.keys(dirtyFields) as (keyof PostFormData)[];
            ...
            // Upload to Supabase Bucket if new file exists
            if (selectedFile) {
                toast.loading('Uploading image...', { id: 'uploadToast' });
                const finalImageUrl = await uploadFile(selectedFile);
                partialData.imageUrl = finalImageUrl;
                toast.success('Image uploaded!', { id: 'uploadToast' });
            }

            if (Object.keys(partialData).length === 0) {
                toast('No changes detected', { icon: 'ℹ️', id: 'uploadToast' });
            ...
            toast.error(err.message || 'Failed to update post. Please try again.', { id: 'uploadToast' });
            setIsSubmitting(false);
        }
    };
                ...
                <div className="flex-1 overflow-y-auto p-12">
                    <div className="max-w-4xl mx-auto">
                        ...
                        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-8 shadow-none">
                            ...
                            {isLoading ? (
                                <p className="text-sm font-bold text-gray-500 uppercase tracking-widest text-center py-12">Loading post data...</p>
                            ) : (
                                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Update Featured Image (Optional)</label>
